...
If the default Tesla `ReverseProxyPlug.HTTPClient` adapter is to be used,
you should also add `:tesla` as an explicit dependency to your project. For example:
...
The default underlying HTTPClient is HTTPoison. However, an implementation for Tesla is
also provided through `ReverseProxyPlug.HTTPClient.Adapters.Tesla`. You can also provide a custom implementation by implementing the `ReverseProxyPlug.HTTPClient` behaviour.

For setting a different adapter, you should add it to your configuration files. For example, setting the Tesla adapter:
